...
used in tests/test_collection.py.
"""
...
def test_get_watchlist_defaults_to_newest_first(app, sample_user):
    """
    get_watchlist() with no sort argument should return films sorted by
    date_added descending (most recently added first), matching get_collection().
    """
    with app.app_context():
...
def test_get_watchlist_sort_by_title(app, sample_user):
    ...
    with app.app_context():
        ...
        # Add Blade Runner most recently, so date-order would put it first.
        entry_a = WatchlistEntry(user_id=sample_user, film_id=film_a.id, date_added=earlier)
        entry_b = WatchlistEntry(user_id=sample_user, film_id=film_b.id, date_added=later)
        db.session.add_all([entry_a, entry_b])
        db.session.commit()
        ...
        # Alphabetical order should win over date order here
